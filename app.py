...
# -----------------------
# Data Loading & Caching (HIGH PERFORMANCE VERSION)
# -----------------------

@st.cache_data(show_spinner=False, ttl=3600)
def load_csv(path: str) -> pd.DataFrame:
    """
    Optimized CSV loader for very large files (500k–3M+ rows)
    """
    chunk_size = 250_000
    chunks = []
    ...
    for chunk in pd.read_csv(
        ...
        on_bad_lines='skip',
        dtype=str,              # prevents dtype fragmentation
        low_memory=False        # more stable parsing
    ):
        chunks.append(chunk)
...
def load_uploaded_file(uploaded_file) -> pd.DataFrame:
    """
    Optimized uploaded file reader
    Avoids memory duplication from .getvalue()
    """
    chunk_size = 250_000
    chunks = []
...
def smart_load():
    ...
    if uploaded is not None:
        with st.spinner("Parsing uploaded CSV (large file safe mode)..."):
...
# -----------------------
# Robust cleaning + derived columns (Optimized)
# -----------------------
...
def clean_and_derive(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # -----------------------
    # Numeric parsing (memory efficient)
    # -----------------------
    ...
    for c in numeric_cols:
        if c in d.columns:
            d[c] = pd.to_numeric(
                d[c].str.replace(',', '', regex=False),
                errors='coerce'
            ).fillna(0).astype('float32')   # smaller memory footprint

    # -----------------------
    # Derived columns
    # -----------------------
    if 'NET_SALES' in d.columns and 'VAT_AMT' in d.columns:
    ...
    if 'STORE_NAME' in d.columns and 'CASHIER' in d.columns:
        d['CASHIER-COUNT'] = d['CASHIER'] + '-' + d['STORE_NAME']
...
# -----------------------
# Aggregation helpers (unchanged but safe)
# -----------------------
