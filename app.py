...
# - Average Customers Served per Till aligned to same 30-min grid & logic
# Usage:
...
def smart_load():
    ...
    uploaded = st.sidebar.file_uploader(
        "Upload DAILY_POS_TRN_ITEMS CSV", type=["csv"]
    )
    ...
    try:
        ...
        st.sidebar.warning(
            "No default CSV found. Please upload a CSV to run the app."
        )
        return None
...
def clean_and_derive(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "TILL" in d.columns and "STORE_CODE" in d.columns:
        d["Till_Code"] = (
            d["TILL"].astype(str) + "-" + d["STORE_CODE"].astype(str)
        )

    if "STORE_NAME" in d.columns and "CASHIER" in d.columns:
        d["CASHIER-COUNT"] = (
            d["CASHIER"].astype(str) + "-" + d["STORE_NAME"].astype(str)
        )
...
@st.cache_data
def agg_count_distinct(
    df: pd.DataFrame, group_by: list, agg_col: str, agg_name: str
) -> pd.DataFrame:
    g = (
        df.groupby(group_by)
...
def format_and_display(
    ...
    if numeric_cols is None:
        numeric_cols = list(
            df_display.select_dtypes(include=[np.number]).columns
        )
    ...
    if index_col and index_col in df_display.columns:
        ...
        non_numeric_cols = [
            c for c in df_display.columns if c not in numeric_cols
        ]
        label_col = (
            non_numeric_cols[0]
            if non_numeric_cols
            else df_display.columns[0]
        )
...
def sales_by_shift(df):
    ...
    fig = go.Figure(
        data=[go.Pie(labels=labels, values=g["NET_SALES"], hole=0.65)]
    )
    fig.update_layout(
...
def night_vs_day_ratio(df):
    ...
    ratio = (
        df_nd.groupby(["STORE_NAME", "Shift_Bucket"])["NET_SALES"]
        .sum()
        .reset_index()
    )
    ratio["STORE_TOTAL"] = ratio.groupby("STORE_NAME")[
        "NET_SALES"
    ].transform("sum")
    ...
    numbered_labels = [
        f"{i+1}. {s}" for i, s in enumerate(pivot_sorted.index)
    ]
    fig = go.Figure()
    fig.add_trace(
        go.Bar(
    ...
    for i, (n_val, d_val) in enumerate(
        zip(pivot_sorted["Night"], pivot_sorted["Day"])
    ):
        fig.add_annotation(
            x=n_val + 1,
            y=numbered_labels[i],
    ...
    st.plotly_chart(fig, use_container_width=True)
    table = pivot_sorted.reset_index().rename(
        columns={"Night": "Night_%", "Day": "Day_%"}
    )
    format_and_display(
        table,
...
def global_day_vs_night(df):
    ...
    if df_nd.empty:
        ...
        return
    agg = (
        df_nd.groupby("Shift_Bucket", as_index=False)["NET_SALES"]
        .sum()
    )
    agg["PCT"] = 100 * agg["NET_SALES"] / agg["NET_SALES"].sum()
    labels = [
        f"{r.Shift_Bucket} ({r.PCT:.1f}%)" for _, r in agg.iterrows()
    ]
    fig = go.Figure(
        go.Pie(labels=labels, values=agg["NET_SALES"], hole=0.65)
    )
    fig.update_layout(
...
def second_highest_channel_share(df):
    ...
    if not all(
        col in df.columns
        for col in ["STORE_NAME", "SALES_CHANNEL_L1", "NET_SALES"]
    ):
        st.warning("Missing columns required")
        return
    data = df.copy()
    store_chan = (
        data.groupby(["STORE_NAME", "SALES_CHANNEL_L1"], as_index=False)[
            "NET_SALES"
        ]
        .sum()
    )
    ...
    store_chan = store_chan.sort_values(
        ["STORE_NAME", "PCT"], ascending=[True, False]
    )
    store_chan["RANK"] = (
        store_chan.groupby("STORE_NAME").cumcount() + 1
    )
    second = store_chan[store_chan["RANK"] == 2][
        ["STORE_NAME", "SALES_CHANNEL_L1", "PCT"]
    ].rename(
        columns={
            "SALES_CHANNEL_L1": "SECOND_CHANNEL",
            "PCT": "SECOND_PCT",
        }
    )
    ...
    format_and_display(
        second_sorted[
            ["STORE_NAME", "SECOND_CHANNEL", "SECOND_PCT"]
        ],
...
def bottom_30_2nd_highest(df):
    ...
    bottom_30 = ranking.sort_values(
        "SECOND_PCT", ascending=True
    ).head(30)
    if bottom_30.empty:
        st.info("No stores to display")
        return
...
def stores_sales_summary(df):
    ...
    if "STORE_NAME" not in df.columns:
        ...
        return
    df2 = df.copy()
    df2["NET_SALES"] = pd.to_numeric(
        df2.get("NET_SALES", 0), errors="coerce"
    ).fillna(0)
    df2["VAT_AMT"] = pd.to_numeric(
        df2.get("VAT_AMT", 0), errors="coerce"
    ).fillna(0)
    df2["GROSS_SALES"] = df2["NET_SALES"] + df2["VAT_AMT"]
    sales_summary = (
        df2.groupby("STORE_NAME", as_index=False)[
    ...
    sales_summary["% Contribution"] = (
        sales_summary["GROSS_SALES"]
        / sales_summary["GROSS_SALES"].sum()
        * 100
    ).round(2)
    if "CUST_CODE" in df2.columns and df2["CUST_CODE"].astype(bool).any():
        ...
        sales_summary = sales_summary.merge(
            cust_counts, on="STORE_NAME", how="left"
        )
    format_and_display(
        sales_summary[
            [
...
def customer_traffic_storewise(df):
    ...
    if "TRN_DATE" not in df.columns or "STORE_NAME" not in df.columns:
        st.warning(
            "Missing TRN_DATE or STORE_NAME — cannot compute traffic."
        )
        return
    ...
    if (
        "CUST_CODE" in d.columns
        and d["CUST_CODE"].astype(str).str.strip().astype(bool).any()
    ):
        ...
        if not all(c in d.columns for c in required_parts):
            st.warning(
                "Missing CUST_CODE and/or its components (STORE_CODE, TILL, SESSION, RCT)."
            )
            return
    ...
    first_touch = (
        d.groupby(
            ["STORE_NAME", "TRN_DATE_ONLY", "CUST_CODE"], as_index=False
        )["TRN_DATE"]
        .min()
    )
    ...
    intervals = [
        (start_time + timedelta(minutes=30 * i)).time()
        for i in range(48)
    ]
    ...
    counts = (
        first_touch.groupby(["STORE_NAME", "TIME_ONLY"])[
            "CUST_CODE"
        ]
        .nunique()
...
def active_tills_during_day(df):
    ...
    if missing:
        st.warning(
            f"Missing required columns for Active Tills view: {missing}"
        )
        return
...
def avg_customers_per_till(df):
    ...
    # Use same alignment & logic as traffic + active tills
    required = ["TRN_DATE", "STORE_NAME", "Till_Code", "TIME_ONLY"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        st.warning(
            f"Missing required columns for Average Customers per Till: {missing}"
        )
        return

    d = df.copy()
    d["TRN_DATE"] = pd.to_datetime(d["TRN_DATE"], errors="coerce")
    d = d.dropna(subset=["TRN_DATE", "STORE_NAME", "Till_Code"]).copy()
    ...
    if (
        ...
        parts = ["STORE_CODE", "TILL", "SESSION", "RCT"]
        if not all(c in d.columns for c in parts):
            st.warning(
                "Missing CUST_CODE and/or its components (STORE_CODE, TILL, SESSION, RCT)."
            )
            return
    ...
    # One 30-min slot per receipt per store/day using earliest time
    d["TRN_DATE_ONLY"] = d["TRN_DATE"].dt.date
    first_touch = (
        d.groupby(
            ["STORE_NAME", "TRN_DATE_ONLY", "CUST_CODE"], as_index=False
    ...
    # Customers = unique receipts per slot
    cust_counts = (
        first_touch.groupby(["STORE_NAME", "TIME_ONLY"])[
            "CUST_CODE"
        ]
    ...
    # Tills = unique active tills per slot
    till_counts = (
        d.groupby(["STORE_NAME", "TIME_ONLY"])["Till_Code"]
        .nunique()
    ...
    if cust_counts.empty or till_counts.empty:
        st.info("No data available to compute average customers per till.")
        return

    cust_pivot = cust_counts.pivot(
        index="STORE_NAME",
        columns="TIME_ONLY",
        values="CUSTOMERS",
    ).fillna(0)
    till_pivot = till_counts.pivot(
        index="STORE_NAME",
        columns="TIME_ONLY",
        values="TILLS",
    ).fillna(0)
    ...
    # Average customers per active till
    ...
    if ratio.empty:
        st.info("No ratio data to display.")
        return

    # Sort by peak ratio
    ratio["MAX"] = ratio.max(axis=1).astype(int)
    ratio = ratio.sort_values("MAX", ascending=False)

    max_vals = ratio["MAX"]
    heatmap_matrix = ratio.drop(columns=["MAX"])
    ...
    fig.update_layout(
        title="Average Customers Served per Till (30-min slots)",
        xaxis_title="Time of Day",
        yaxis_title="",
        height=max(600, 25 * len(heatmap_matrix.index)),
        margin=dict(l=160, r=20, t=80, b=40),
    ...
    # Summary: peak average customers per till per store
    pivot_totals = pd.DataFrame(
        {
            "STORE_NAME": heatmap_matrix.index,
            "MAX_CUSTOMERS_PER_TILL": max_vals.values,
        }
    )
    st.subheader("Peak Average Customers per Till per Store")
    format_and_display(
        pivot_totals,
...
def store_customer_traffic_storewise(df):
    ...
    if d.empty:
        ...
        return
    tmp = (
        d.groupby(["DEPARTMENT", "TIME_ONLY"])["CUST_CODE"]
        .nunique()
...
def cashiers_performance(df):
    ...
    required = ["TRN_DATE", "CUST_CODE", "CASHIER-COUNT", "STORE_NAME"]
    if not all(c in df.columns for c in required):
        st.warning("Missing required columns for cashier performance")
        return
    d = df.copy()
    receipt_duration = d.groupby(
        ["STORE_NAME", "CUST_CODE"], as_index=False
    ).agg(
        Start_Time=("TRN_DATE", "min"),
        End_Time=("TRN_DATE", "max"),
    )
    receipt_duration["Duration_Sec"] = (
        receipt_duration["End_Time"]
        - receipt_duration["Start_Time"]
    ...
    cashier_summary = merged.groupby(
        ["STORE_NAME", "CASHIER-COUNT"], as_index=False
    ).agg(
        Avg_Duration_Sec=("Duration_Sec", "mean"),
        Customers_Served=("CUST_CODE", "nunique"),
    )
    cashier_summary["Avg_Serve_Min"] = (
        cashier_summary["Avg_Duration_Sec"] / 60
    ).round(1)
    branches = sorted(cashier_summary["STORE_NAME"].unique())
    branch = st.selectbox(
        "Select Branch for Cashier Performance", branches
    )
    dfb = cashier_summary[
        cashier_summary["STORE_NAME"] == branch
    ].sort_values("Avg_Serve_Min")
    if dfb.empty:
...
def till_usage(df):
    ...
    till_activity = d.groupby(
        ["STORE_NAME", "Till_Code", "TIME_ONLY"], as_index=False
...
def tax_compliance(df):
    ...
    store_till = d.groupby(
        ["STORE_NAME", "Till_Code", "Tax_Compliant"], as_index=False
    ).agg(Receipts=("CUST_CODE", "nunique"))
    branch = st.selectbox(
    ...
    pivot = pivot.reindex(
        columns=["Compliant", "Non-Compliant"], fill_value=0
    )
    fig = go.Figure()
    fig.add_trace(
        go.Bar(
    ...
    store_summary = (
        d.groupby(
            ["STORE_NAME", "Tax_Compliant"], as_index=False
        ).agg(Receipts=("CUST_CODE", "nunique"))
        .pivot(
            index="STORE_NAME",
            columns="Tax_Compliant",
...
def customer_baskets_overview(df):
    ...
    d = df.copy()
    d = d.dropna(
        subset=["ITEM_NAME", "CUST_CODE", "STORE_NAME", "DEPARTMENT"]
    )
    d = d[~d["DEPARTMENT"].str.upper().eq("LUGGAGE & BAGS")]
    branches = sorted(d["STORE_NAME"].unique())
    branch = st.selectbox(
        "Branch for comparison (branch vs global)", branches
    )
    metric = st.selectbox("Metric", ["QTY", "NET_SALES"])
    top_x = st.number_input(
        "Top X", min_value=5, max_value=200, value=10
    )
    ...
    basket_count = temp.groupby("ITEM_NAME")[
        "CUST_CODE"
    ].nunique().rename("Count_of_Baskets")
    agg_data = temp.groupby("ITEM_NAME")[
        ["QTY", "NET_SALES"]
    ].sum()
    global_top = (
    ...
    if branch_df.empty:
        st.info("No data for selected branch")
        return
    basket_count_b = branch_df.groupby("ITEM_NAME")[
        "CUST_CODE"
    ].nunique().rename("Count_of_Baskets")
    agg_b = branch_df.groupby("ITEM_NAME")[
        ["QTY", "NET_SALES"]
    ].sum()
    branch_top = (
    ...
    missing = set(global_top["ITEM_NAME"]) - set(
        branch_top["ITEM_NAME"]
    )
    if missing:
        st.subheader(
            f"Items in Global Top {top_x} but missing/underperforming in {branch}"
        )
        missing_df = global_top[
            global_top["ITEM_NAME"].isin(missing)
        ...
        st.success(
            f"All top {top_x} global items also present in {branch}"
        )
...
def branch_comparison(df):
    ...
    with col2:
        b = st.selectbox(
            "Branch B", branches, index=1 if len(branches) > 1 else 0
        )
    metric = st.selectbox("Metric", ["QTY", "NET_SALES"])
    top_x = st.number_input(
        "Top X", min_value=5, max_value=200, value=10
    ...
    def top_items(branch):
        ...
        baskets = temp.groupby("ITEM_NAME")[
            "CUST_CODE"
        ].nunique().rename("Count_of_Baskets")
        totals = temp.groupby("ITEM_NAME")[
            ["QTY", "NET_SALES"]
        ].sum()
        merged = (
            baskets.to_frame()
...
def product_performance(df):
    ...
    baskets = (
        item_data[["STORE_NAME", "CUST_CODE"]]
        .drop_duplicates()
        .assign(Has_Item=1)
    )
    basket_counts = (
        baskets.groupby("STORE_NAME")
        .agg(Baskets_With_Item=("Has_Item", "sum"))
        .reset_index()
    )
    total_qty = (
        item_data.groupby("STORE_NAME")["QTY"]
        .sum()
        .reset_index()
        .rename(columns={"QTY": "Total_QTY_Sold_Branch"})
    )
    summary = (
...
def global_loyalty_overview(df):
    ...
    dfL = dfL.dropna(
        subset=["TRN_DATE", "STORE_NAME", "CUST_CODE"]
    )
...
def branch_loyalty_overview(df):
    st.header(
        "Branch Loyalty Overview (per-branch loyal customers with >1 baskets)"
    )
    if not all(
        c in df.columns
        for c in [
...
def customer_loyalty_overview(df):
    ...
    if not customers:
        ...
        return
    cust = st.selectbox(
        "Select Loyalty Customer (multi-store)", customers
    )
    ...
    per_store = rc.groupby("STORE_NAME", as_index=False).agg(
        ...
        ["Baskets", "Total_Value"], ascending=False
    )
    format_and_display(
        per_store,
...
def global_pricing_overview(df):
    ...
    multi_price = grp[
        (grp["Num_Prices"] > 1) & (grp["Price_Spread"] > 0)
    ].copy()
...
def branch_pricing_overview(df):
    ...
    per_item_day = d.groupby(
        ["DATE", "ITEM_CODE", "ITEM_NAME"], as_index=False
    ).agg(
        Num_Prices=("SP_PRE_VAT", lambda s: s.dropna().nunique()),
        Price_Min=("SP_PRE_VAT", "min"),
        Price_Max=("SP_PRE_VAT", "max"),
...
def global_refunds_overview(df):
    ...
    summary = neg.groupby(
        ["STORE_NAME", "Sale_Type"], as_index=False
    ).agg(
...
def main():
    ...
    if section == "SALES":
        ...
        choice = st.sidebar.selectbox(
            "Operations Subsection", ops_items
        )
