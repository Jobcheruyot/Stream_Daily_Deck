...
def pick_sales_column(df: pd.DataFrame) -> str | None:
    """
...
def ensure_column(df: pd.DataFrame, col: str) -> bool:
    if col not in df.columns:
        st.info(f"Column `{col}` not found in dataset. "
...
def sales_channel_distribution(df: pd.DataFrame) -> None:
    ...
    if "SALES_CHANNEL_L2" not in df.columns:
        st.info("Column `SALES_CHANNEL_L2` not found. "
...
def sales_night_vs_day(df: pd.DataFrame) -> None:
    ...
    has_night = (
        df[df["SHIFT"].str.upper() == "NIGHT"]["STORE_NAME"]
        .dropna()
        .unique()
    )
    ...
    sub["DAY_NIGHT"] = np.where(
        sub["SHIFT"].str.upper() == "NIGHT", "NIGHT", "DAY"
    )
...
# ============================================================================
# OPERATIONS / INSIGHTS (simple placeholders)
# ============================================================================
